...
function validateDateFields() {
  ...
  else if ([4, 6, 9, 11].includes(monthValue) && dayValue > 30) {
    isValid = handleValidation(
      dayInput,
      dayLabel,
      dayError,
  ...
  // Validate February
    ...
    if (
      (!isLeapYear(yearValue) && dayValue > 28) ||
      (isLeapYear(yearValue) && dayValue > 29)
    ) {
      isValid = handleValidation(
        dayInput,
        dayLabel,
  ...
  // Validate year (must not be in the future)
  isValid =
    handleValidation(
      yearInput,
      yearLabel,
...
calculateButton.addEventListener("click", function () {
  ...
  if (isDayValid && isMonthValid && isYearValid && validateDateFields()) {
    ...
    console.log("Validation failed, no calculation.");
  }
});
